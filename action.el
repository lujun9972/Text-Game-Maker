...
使用'watch 物品'查看指定物品"
  ...
  (let ((object (or (unless symbol tg-current-room)
                    ...
                    (tg-get-inventory-by-symbol symbol t)
                    (tg-get-creature-by-symbol symbol))))
...
(tg-defaction tg-use (inventory)
  ...
  (unless (tg-inventory-usable-p inventory)
    (throw 'exception (format "%s不可使用" inventory)))
  (let ((object (tg-get-inventory-by-symbol inventory)))
    (tg-run-trigger Inventory-use-trigger object)
    (tg-take-effects-to-creature tg-myself (Inventory-effects object))
    (tg-remove-inventory-from-creature tg-myself inventory)))

(tg-defaction tg-wear (equipment)
  "使用'wear 物品'装备自己随身携带的equipment"
  (unless (tg-inventory-exist-in-creature-p tg-myself equipment)
    (throw 'exception (format "未携带%s" equipment)))
  (unless (tg-inventory-wearable-p equipment)
    (throw 'exception (format "%s不可装备" equipment)))
  (let ((object (tg-get-inventory-by-symbol equipment)))
    (tg-run-trigger Inventory-wear-trigger object)
    (tg-take-effects-to-creature tg-myself (Inventory-effects object))
    (tg-remove-inventory-from-creature tg-myself equipment)
    (tg-add-equipment-to-creature tg-myself equipment)
...
(tg-defaction tg-attack (target)
  ...
  (let* ((target-creature (tg-get-creature-by-symbol target))
         (my-attack (or (cdr (assoc 'attack (Creature-attr tg-myself))) 0))
         (my-defense (or (cdr (assoc 'defense (Creature-attr tg-myself))) 0))
    ...
    (tg-take-effect-to-creature target-creature (cons 'hp (- damage)))
    ...
    (if (<= (cdr (assoc 'hp (Creature-attr target-creature))) 0)
        (progn
          ...
          (let ((exp-gained (tg-get-exp-reward target-creature)))
            (tg-display (format "获得 %d 点经验值！" exp-gained))
            (tg-add-exp-to-creature tg-myself exp-gained)))
      (let* ((counter-damage (max 1 (- target-attack my-defense))))
        (tg-take-effect-to-creature tg-myself (cons 'hp (- counter-damage)))
        (tg-display (format "%s反击，造成 %d 点伤害！" target counter-damage))
        (if (<= (cdr (assoc 'hp (Creature-attr tg-myself))) 0)
            (progn
              (tg-display "你被击败了！游戏结束！")
              (setq tg-over-p t))
          (tg-display (format "你的HP: %d | %s的HP: %d"
                              (cdr (assoc 'hp (Creature-attr tg-myself)))
                              target
                              (cdr (assoc 'hp (Creature-attr target-creature))))))))))

(tg-defaction tg-upgrade (attr points)
  "使用'upgrade <属性> <点数>'消耗技能点提升指定属性"
  (unless (assoc 'bonus-points (Creature-attr tg-myself))
    (throw 'exception "没有bonus-points属性"))
  (unless (assoc attr (Creature-attr tg-myself))
    (throw 'exception (format "没有%s属性，无法分配" attr)))
  (let ((pts (string-to-number (if points (format "%s" points) "0")))
        (available (cdr (assoc 'bonus-points (Creature-attr tg-myself)))))
    (unless (> pts 0)
      (throw 'exception "请输入有效的点数"))
    (unless (>= available pts)
      (throw 'exception "技能点不足"))
    (cl-incf (cdr (assoc attr (Creature-attr tg-myself))) pts)
    (cl-decf (cdr (assoc 'bonus-points (Creature-attr tg-myself))) pts)
    (tg-display (format "分配 %d 点到 %s，剩余技能点: %d" pts attr (cdr (assoc 'bonus-points (Creature-attr tg-myself)))))))
