...
(defun play-sample-game ()
  ...
  (let ((sample-dir sample-game-dir))
    ...
    (tg-creatures-init (expand-file-name "creature-config.el" sample-dir))
    (tg-level-init (expand-file-name "level-config.el" sample-dir))
    (tg-quest-init (expand-file-name "quest-config.el" sample-dir))
    (tg-dialog-init (expand-file-name "dialog-config.el" sample-dir))
    (tg-shop-init (expand-file-name "shop-config.el" sample-dir))
    (setq tg-player-gold 20)
    (tg-mode)
