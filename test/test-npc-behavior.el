...
(ert-deftest test-npc-condition-player-in-room-true ()
  ...
  (test-with-globals-saved (tg-rooms-alist tg-room-map tg-current-room creatures-alist myself)
...
(ert-deftest test-npc-run-behaviors-matches-first-rule ()
  ...
  (test-with-globals-saved (tg-display-fn creatures-alist myself tg-rooms-alist tg-room-map tg-current-room tg-over-p)
    (let* ((goblin (make-Creature :symbol 'goblin :attr '((hp . 25) (attack . 5))
