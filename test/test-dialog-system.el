...
(ert-deftest test-dialog-evaluate-condition-and ()
  ...
  (test-with-globals-saved (quests-alist tg-myself)
    (let ((q (make-Quest :symbol 'test-q :status 'active))
...
(ert-deftest test-dialog-handle-choice-applies-effects ()
  ...
  (test-with-globals-saved (dialog-pending tg-display-fn tg-level-exp-table tg-level-up-bonus-points tg-auto-upgrade-attrs)
